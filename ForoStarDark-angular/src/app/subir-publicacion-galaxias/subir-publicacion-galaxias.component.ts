...
export class SubirPublicacionGalaxiasComponent {
  tituloSeccion: string = 'GALAXIAS'; // Título de la sección
  tituloTexto: string = ''; // Input del usuario
  descripcionTexto: string = ''; // Input del usuario
  ...
  onFileSelected(event: any) {
    ...
    if (file) {
      this.fileType = file.type.split('/')[0]; // Imagen, video, etc.
  ...
  postPublication() {
    ...
    this.authService.savePublication(nuevaPublicacion, 'publicaciones_galaxias');
  ...
  cargarPublicaciones() {
    const publicaciones = JSON.parse(localStorage.getItem('publicaciones_galaxias') || '[]');
