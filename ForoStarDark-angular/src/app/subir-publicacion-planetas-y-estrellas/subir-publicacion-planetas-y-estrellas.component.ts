...
export class SubirPublicacionPlanetasYEstrellasComponent {
  tituloSeccion: string = 'PLANETAS Y ESTRELLAS'; // Sección fija
  ...
  postPublication() {
    ...
    this.authService.savePublication(nuevaPublicacion, 'publicaciones_planetas_estrellas');
  ...
  cargarPublicaciones() {
    const publicaciones = JSON.parse(localStorage.getItem('publicaciones_planetas_estrellas') || '[]');
