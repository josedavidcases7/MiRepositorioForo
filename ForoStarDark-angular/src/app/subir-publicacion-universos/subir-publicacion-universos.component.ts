...
export class SubirPublicacionUniversosComponent {
  tituloSeccion: string = 'UNIVERSOS'; // Título fijo de la sección
  ...
  postPublication() {
    ...
    this.authService.savePublication(nuevaPublicacion, 'publicaciones_universos');
  ...
  cargarPublicaciones() {
    const publicaciones = JSON.parse(localStorage.getItem('publicaciones_universos') || '[]');
