...
export class SubirPublicacionVidaExtraterrestreComponent {
  tituloSeccion: string = 'VIDA EXTRATERRESTRE'; // Título fijo de la sección
  ...
  postPublication() {
    ...
    this.authService.savePublication(nuevaPublicacion, 'publicaciones_vida_extraterrestre');
  ...
  cargarPublicaciones() {
    const publicaciones = JSON.parse(localStorage.getItem('publicaciones_vida_extraterrestre') || '[]');
